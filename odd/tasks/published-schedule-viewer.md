...
## Tasks
- [x] **PSV-1 — Add single-PDF Drive bridge and server data path.** Added a dedicated `PublishedSchedule.gs` Apps Script bridge and a TanStack server function. The bridge rejects empty/multiple/non-PDF folder contents; the server forwards its secret only in a server-side POST and returns validated status, filename, and viewer URL. Structural readback confirmed the secret is not included in the client result and the existing bridge files are unchanged. Commit: `96b2411` (`feat(schedules): add Drive PDF bridge`).
- [x] **PSV-2 — Add viewer route, navigation, and coordinator setup guide.** Added `/horarios` with the current PDF inline and no app download/archive actions; linked it from home and mobile navigation; added local Apps Script/Drive deployment and replacement steps. Typecheck and build passed; focused ESLint passed; full lint remained incomplete after stalling. Structural verification found no concrete defects. Commit: `ef07d41` (`feat(schedules): add public schedule viewer`).
...
- PSV-1 closed in commit `96b2411`; PSV-2 closed in commit `ef07d41`. `npm.cmd run typecheck` passed as the parent spot check; the writer reported `npm.cmd run build` and changed-file ESLint passed. Full `npm.cmd run lint` did not complete after two stalled attempts and remains partial. An independent read-only verifier found no concrete defects. `gentle-ai review assess --cwd . --json` could not classify the candidate because it contained undeclared untracked files; RDD is off, so no review lifecycle was started.

## Next Step
Implementation is complete. The only pending verification is a successful full-repository lint run; the attempted runs stalled without returning a result.
